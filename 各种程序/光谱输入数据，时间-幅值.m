...
path = 'C:\Users\酒仙桥科研巨擘专用\Desktop\频谱仪实验数据\500个数据\第1到第377';           %添加数据文件夹绝对路径,工作区间要在同一个盘内
...
for i = 1:length(fileNames) %加载数据，放入矩阵数组
    ...
    data_in =csvread(fileNames{i},10); %修改数值从不同行开始读取
 %———判断前k行属于无用数据———————————
...
title([fileNames{i} 'ACF '],'fontsize',18,'fontweight','bold','fontname','微软雅黑','Color','r');
xlabel('Time(s)','fontname','微软雅黑','fontsize',14,'FontWeight','bold','Color','r'); 
ylabel('Amplitude(???)','fontname','微软雅黑','fontsize',14,'FontWeight','bold','Color','r');
 %————————存图———d—————————————
 mkdir csv_image;           %新建dat_image文件夹，如存在会警告，不影响程序
 filepath=pwd;              %保存当前工作目录
 cd('C:\Users\酒仙桥科研巨擘专用\Desktop\新建文件夹 (6)');    %把当前工作目录切换到指定文件夹
 f = getframe(gcf);
