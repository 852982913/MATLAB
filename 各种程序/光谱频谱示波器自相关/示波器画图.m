...
title([fileNames{i} '示波器'],'fontsize',18,'fontweight','bold','fontname','微软雅黑','Color','r');
xlabel('Relative Time (s)','fontname','微软雅黑','fontsize',14,'FontWeight','bold','Color','r'); 
ylabel('Intensity (mV)','fontname','微软雅黑','fontsize',14,'FontWeight','bold','Color','r');
 %————————存图————————————————
 mkdir csv_image;           %新建dat_image文件夹，如存在会警告，不影响程序
 filepath=pwd;              %保存当前工作目录
 cd('C:\Users\酒仙桥科研巨擘专用\Desktop\示波器\csv_image');    %把当前工作目录切换到指定文件夹
 f = getframe(gcf);
imwrite(f.cdata,[fileNames{i} '示波器.png']);%将fileNames{i} 变量写入文件名
 cd(filepath); %返回工作目录
